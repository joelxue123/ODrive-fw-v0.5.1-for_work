...
void ASCII_protocol_process_line(const uint8_t* buffer, size_t len, StreamSink& response_channel) {
    ...
    if (cmd[0] == 'p') { // position control
        ...
        } else if (motor_number >= AXIS_COUNT_USED) {
        ...
        if (numscan < 1) {
            ...
            respond(response_channel, use_checksum, "%f %f",
                    (double)axes[motor_number]->encoder_.pos_estimate_.any().value_or(0.0f),
                    (double)axes[motor_number]->encoder_.vel_estimate_.any().value_or(0.0f));
        }
